...
const SPELLS = [
    {
        ...
        BOSS: 'Cupid\'s Shadow', color: '#ff00aa', // Neon Pink (High Vis)
        sheet: 'INGR_1',
...
const SHEET_SRC = {
    ...
    INGR_1: '/images/craft-work/ingredients_sheet_1.png',
    INGR_2: '/images/craft-work/ingredients_sheet_2.png',
...
function init() {
    ...
    SPELLS.forEach(spell => {
        ...
        div.innerHTML = `<div style="color:${spell.color};font-size:24px">âœ¨</div>${spell.name}<br><span style="font-size:9px">${spell.id.toUpperCase()}</span>`;
    ...
    for (let y = 0; y < MAP_HEIGHT; y++) {
        ...
        for (let x = 0; x < MAP_WIDTH; x++) {
            ...
            if (type === TILES.GRASS) {
                ...
                else if(r < 0.06) deco = 'ðŸ¡';
                else if(r < 0.08) deco = 'ðŸª¨';
                else if(r < 0.15) deco = 'ðŸŒ¿';
            }
            if (type === TILES.SNOW && r < 0.05) deco = SPRITES.ROCK;
            if (type === TILES.SAND && r < 0.05) deco = 'ðŸŒµ';
            if (type === TILES.SWAMP && r < 0.05) deco = 'ðŸ„';
...
function spawnEnemy(x, y, type) {
    ...
    if (type === 'boss') {
        if(gameState.selectedSpell.id === 'love') spriteData = SPRITES.BOSS_LOVE;
        else if(gameState.selectedSpell.id === 'wealth') spriteData = SPRITES.BOSS_WEALTH;
        else if(gameState.selectedSpell.id === 'health') spriteData = SPRITES.BOSS_HEALTH;
        else if(gameState.selectedSpell.id === 'glamour') spriteData = SPRITES.BOSS_GLAMOUR;
    } else {
        // Minion progression: Slime -> Bat -> Spider -> Ghost -> Goblin -> Skeleton
        const order = [SPRITES.SLIME, SPRITES.BAT, SPRITES.SPIDER, SPRITES.GHOST, SPRITES.GOBLIN, SPRITES.SKELETON];
        spriteData = order[Math.min(stage, order.length-1)];
    }
...
function playerAttack() {
    if (gameState.screen !== 'PLAY' || !gameState.selectedSpell) return;
    if (player.isAttacking) return; // Cooldown/Animation Lock
    ...
    if (joystickInput.active && (Math.abs(joystickInput.x) > 0.1 || Math.abs(joystickInput.y) > 0.1)) {
        // Shoot in joystick direction
        const mag = Math.sqrt(joystickInput.x*joystickInput.x + joystickInput.y*joystickInput.y);
        vx = (joystickInput.x / mag) * 12;
        vy = (joystickInput.y / mag) * 12;
    } else {
        // Keyboard/Fallback direction logic (support diagonals)
        let kx = 0, ky = 0;
        ...
        if (kx !== 0 || ky !== 0) {
            ...
            // Static facing fallback
            // Static facing fallback
            const speed = 12; 
            let dir = player.facing || 'down';
            
            // Support Diagonal Firing based on 'facing' set in movement loop
            switch(dir) {
                case 'up': vy = -speed; break;
                case 'down': vy = speed; break;
                case 'left': vx = -speed; break;
                case 'right': vx = speed; break;
                case 'up-left': vx = -speed * 0.7; vy = -speed * 0.7; break;
                case 'up-right': vx = speed * 0.7; vy = -speed * 0.7; break;
                case 'down-left': vx = -speed * 0.7; vy = speed * 0.7; break;
                case 'down-right': vx = speed * 0.7; vy = speed * 0.7; break;
                default: vy = speed; 
            }
        }
    }
...
function update(dt) {
    ...
    if (gameState.screen === 'PLAY' && player.hp > 0) {
        ...
        if (dx !== 0 || dy !== 0) {
            ...
            // FACING LOGIC FIX
            // Prioritize Left/Right facing over Up/Down for sprites usually
            // But diagonals need care. 
            // Previous logic might have been simplistic.
            if (dx < 0) { player.flipX = true; player.facing = 'left'; }
            if (dx > 0) { player.flipX = false; player.facing = 'right'; }
            if (dy < 0 && dx === 0) player.facing = 'up';
            if (dy > 0 && dx === 0) player.facing = 'down';
            
            // For diagonals, we usually keep left/right flip but might want to know 'up-left' etc for shooting
            if (dy < 0 && dx < 0) player.facing = 'up-left';
            if (dy < 0 && dx > 0) player.facing = 'up-right';
            if (dy > 0 && dx < 0) player.facing = 'down-left';
            if (dy > 0 && dx > 0) player.facing = 'down-right';
        ...
        if (player.animTimer > 5) { // Fast attack
            ...
            if (player.frameX > 3) {
                ...
                // Return to correct walking frame row based on facing
                 ...
                 if(player.facing === 'up') player.frameY = 1;
                 if(player.facing === 'left' || player.facing === 'right') player.frameY = 2;
            }
        }
    }
    }
    ...
    gameState.enemyProjectiles.forEach(ep => {
        ...
        if (rectIntersect(ep.x, ep.y, 10, 10, player.x, player.y, player.w, player.h)) {
            takeDamage(15, "Enemy Projectile");
            ep.life = 0;
        }
    });
...
function respawnPlayer() {
    ...
    // Safety Invulnerability (3 seconds)
    ...
    // Pause game loop logic implicitly by blocking input or state?
    // Actually game loop continues running which is fine, but we might want to pause updates?
    // For now, let world run, player is safe at spawn.
...
function draw() {
    ...
    gameState.enemies.forEach(e => {
        ...
        ctx.fillStyle = 'green'; ctx.fillRect(e.x, e.y - 10, e.w * (e.hp/e.maxHp), 5);
    });
