...
const readerConfigs: Record<string, ReaderConfig> = {
  ambrose: {
    assistantId: "517aca67-ced6-4710-927d-4dd1f944419",
    buttonConfig: {
...
export default function TarotReaderPage() {
  ...
  // THE FIX: Define the click hijacking function for the active call state
  const handleActiveButtonClick = useCallback((event: MouseEvent) => {
  ...
  const startPollingForCards = useCallback(() => {
    // THE FIX: The public method IS getCall(), which returns the call object.
    const call = (vapiRef.current as any)?.getCall();
  ...
  const handleStartCall = async () => {
    ...
    setTimeout(async () => {
      // THE FIX: Use the correct public method here as well.
      const call = (vapiRef.current as any)?.getCall();
      const sessionId = call?.id;
  ...
  const handleEndCall = () => {
    // This is now handled by the event hijacker, but we keep the handler for clarity.
    showWarningPopup();
  };
  ...
  return (
    <main className="relative min-h-screen w-full bg-black bg-cover bg-center" style={{ backgroundImage: "url('/images/grand-hall-bg.png')" }}>
      ...
      <div className="relative z-10">
        <div id="vapi-support-btn" onClick={callStatus === 'idle' ? handleStartCall : undefined}>
