...
export default function BookshelfClient({ books }: BookshelfClientProps) {
  ...
  return (
    <div className="relative z-10 mt-12 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8">
      {books.map((book) => (
        <Link key={book.slug} href={`/the-magick-psychic-school/the-magick-library/${book.slug}`} className="group flex flex-col items-center text-center transition-all duration-300 hover:scale-105 active:scale-95">
