...
const StageTitle: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <h3 className="text-3xl font-serif text-amber-200/90 text-center shrink-0" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>
        {children}
    </h3>
);
...
const RitualButton: React.FC<{ onClick: () => void; children: React.ReactNode; className?: string; disabled?: boolean; }> = ({ onClick, children, className, disabled }) => {
    ...
    return (
        <button
            onClick={handleClick}
            disabled={disabled}
            className={`px-8 py-3 bg-black/40 text-white font-serif rounded-lg border-2 border-purple-400/50 backdrop-blur-sm hover:bg-purple-900/50 hover:border-purple-300 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shrink-0 ${className}`}
        >
            {children}
        </button>
    );
                        ...
                        <Stage className="gap-4">
                            ...
                            <div className="w-full grow min-h-0 flex items-center justify-center">
                                <div className="relative w-full h-full max-w-lg">
                                    {/* THE FIX: Replaced h-auto with h-full */}
                    ...
                    {ritualStep === 1 && (
                        <Stage className="gap-4">
                            ...
                            <div className="w-full grow min-h-0 flex items-center justify-center">
                                <div className="relative w-full h-full max-w-lg">
                                    {/* THE FIX: Replaced h-auto with h-full */}
                                    <Image src={`${ASSET_PATH}/wicca_scroll_intention.png`} width={500} height={625} alt="Inscribe Intention" className="w-full h-full object-contain"/>
                     ...
                     {ritualStep === 2 && (
                        <Stage className="gap-4">
                            ...
                            <div className="w-full grow min-h-0 flex items-center justify-center">
                             ...
                             {chargedElements.length === 5 ? ( <div className="w-full flex justify-center"> <RitualButton onClick={() => setRitualStep(3)} className="animate-pulse">Continue</RitualButton> </div> ) : <div className="h-[52px]" />}
                        </Stage>
                    )}
                    {ritualStep === 3 && (
                    ...
                    {ritualStep === 5 && generatedSpell && (
                         <Stage className="gap-4">
                            ...
                            <div className="w-full grow min-h-0 flex items-center justify-center">
                                <div className="relative w-full h-full max-w-lg">
                                    {/* THE FIX: Replaced h-auto with h-full */}
                                    <Image src={`${ASSET_PATH}/wicca_charge_ingredient_template.png`} width={500} height={625} alt="Charge Ingredient" className="w-full h-full object-contain" />
                                    ...
                                    <div onMouseDown={handleChargeStart} onMouseUp={handleChargeEnd} onMouseLeave={handleChargeEnd} onTouchStart={handleChargeStart} onTouchEnd={handleChargeEnd} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-40 h-40 grid place-items-center cursor-pointer select-none">
                                        <div className={`absolute inset-0 w-full h-full transition-transform duration-300 ${isCharging ? 'scale-110' : 'scale-100'}`}>
                                            {(() => {
                                                ...
                                                return <Sprite sheetPath={spriteData.sheet.path} x={spriteData.itemInfo.x} y={spriteData.itemInfo.y} spriteWidth={spriteData.sheet.spriteSize.width} spriteHeight={spriteData.sheet.spriteSize.height} sheetWidth={spriteData.sheet.sheetSize.width} sheetHeight={spriteData.sheet.sheetSize.height} />;
                                            })()}
                                        </div>
                            ...
                            <div className="w-full flex justify-center h-[52px]">
                    ...
                    {ritualStep === 6 && generatedSpell && (
                        <Stage className="gap-4">
                             ...
                             <div className="w-full grow min-h-0 flex items-center justify-center">
                                <div className="relative w-full h-full max-w-lg">
                                    {/* THE FIX: Replaced h-auto with h-full */}
                                    <Image src={`${ASSET_PATH}/wicca_incantation_scroll.png`} width={500} height={625} alt="Incantation" className="w-full h-full object-contain" />
                         ...
                         <Stage className="gap-4">
                            ...
                            <div className="w-full grow min-h-0 flex items-center justify-center">
                                <div className="relative w-full h-full max-w-2xl">
                                    {/* THE FIX: Replaced h-auto with h-full */}
                                    <Image src={`${ASSET_PATH}/wicca_spell_manifestation.png`} width={800} height={800} alt="Spell Manifestation" className="w-full h-full object-contain" />
