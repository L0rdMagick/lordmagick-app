      ...
      >
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
            ...
            <div className="flex-1 p-6 md:p-8 flex flex-col gap-6 border-b md:border-b-0 md:border-r border-white/10 relative overflow-y-auto custom-scrollbar">
                {/* Alternative Minimize Button (Legacy) */}
                <button 
                  onClick={() => setIsExpanded(false)}
        ...
        {/* ROW 2: DEFINITIONS LEGEND (Footer) */}
        <div className="w-full bg-slate-950/50 border-t border-white/10 p-6 md:px-8 grid grid-cols-1 md:grid-cols-2 gap-8 overflow-y-auto">
           {/* HITTING */}
           <div>
              <h4 className="text-[9px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-3 flex items-center gap-2">
                  <span className="w-1 h-1 bg-indigo-400 rounded-full"></span> Psi-Hitting (Positive)
              </h4>
              <div className="space-y-1.5">
                 {[
                    { title: "The Oracle", z: ">= 4.0", c: "text-amber-400" },
                    { title: "The Medium", z: ">= 3.0", c: "text-purple-400" },
                    { title: "The Clairvoyant", z: ">= 1.96", c: "text-indigo-400" },
                    { title: "The Channel", z: ">= 1.65", c: "text-blue-400" },
                    { title: "The Adept", z: ">= 1.0", c: "text-emerald-400" },
                    { title: "The Spark", z: ">= 0.5", c: "text-teal-400" }
                 ].map(r => (
                     <div key={r.title} className="flex justify-between text-[9px] group cursor-default hover:bg-white/5 p-1 rounded transition-colors">
                        <span className={`font-black uppercase tracking-wider ${r.c} group-hover:scale-105 transition-transform origin-left`}>{r.title}</span>
                        <span className="font-mono font-bold text-slate-600 group-hover:text-slate-400">Z {r.z}</span>
                     </div>
                 ))}
                 <div className="flex justify-between text-[9px] group cursor-default hover:bg-white/5 p-1 rounded transition-colors">
                    <span className="font-bold uppercase tracking-wider text-slate-200">The Initiate</span>
                    <span className="font-mono font-bold text-slate-600 group-hover:text-slate-400">Z &gt; 0</span>
                 </div>
              </div>
           </div>
           ...
           <div>
              <h4 className="text-[9px] font-black uppercase tracking-[0.2em] text-rose-400 mb-3 flex items-center gap-2">
                 <span className="w-1 h-1 bg-rose-400 rounded-full"></span> Psi-Missing (Negative)
              </h4>
              <div className="space-y-1.5">
                 {[
                   { title: "Desynchronized", z: "< 0.0", c: "text-slate-400" },
                   { title: "Signal Noise", z: "<= -0.5", c: "text-orange-400" },
                   { title: "Cognitive Dissonance", z: "<= -1.0", c: "text-red-400" },
                   { title: "Reality Inversion", z: "<= -2.0", c: "text-rose-500" },
                   { title: "The Shadow", z: "<= -3.0", c: "text-fuchsia-500" },
                   { title: "The Void", z: "<= -4.0", c: "text-slate-600" }
                 ].map(r => (
                    <div key={r.title} className="flex justify-between text-[9px] group cursor-default hover:bg-white/5 p-1 rounded transition-colors">
                        <span className={`font-black uppercase tracking-wider ${r.c} group-hover:scale-105 transition-transform origin-left`}>{r.title}</span>
                        <span className="font-mono font-bold text-slate-600 group-hover:text-slate-400">Z {r.z}</span>
